...
{ 
  ...
  private int       lives = 3;      // Lives in the game
  private int       bricksLeft;     // Bricks left
  ...
  public void drawActualPicture( Graphics2D g )
  {
    ...
    synchronized( Model.class )   // Make thread safe
    {
      if (lives == 0)
      {
          ...
          // g.drawString(gameOverMessage, width/2-fm.stringWidth(gameOverMessage)/2, 80);
      ...
      displayGameObjBall( g, ball );    // Display the Ball
  ...
  private void displayGameObjBall( Graphics2D gBall, GameObj go )
  {
      gBall.setColor( go.getColour().forSwing() );
      gBall.fill( new Ellipse2D.Float(go.getX(),     go.getY(), 
                                   go.getWidth(), go.getHeight() ));
  }   
  ...
  private Image background_img;             // Adding Backgrounf Image
  private Image gameover_img;               // Game Over Image
  ...
  {                                          //  to avoid flicker
    ...
    if (  theAG == null )
    {
      ...
      background_img = Toolkit.getDefaultToolkit().createImage("background12.jpg");
      gameover_img = Toolkit.getDefaultToolkit().createImage("gameover.jpg");
